#line 1 "/home/calvin/EC/ESP32Marauder/esp32_marauder/WiFiScan.cpp"
#include "WiFiScan.h"
...
void WiFiScan::setMac() {
  ...
  if ((result != ESP_OK) && ((currentWiFiMode == WIFI_MODE_AP) ||
                             (currentWiFiMode == WIFI_MODE_APSTA) ||
                             (currentWiFiMode == WIFI_MODE_NULL)))
    Serial.printf("Failed to set AP MAC: %s | 0x%X\n",
                  macToString(this->ap_mac), result);
  // else if ((currentWiFiMode == WIFI_MODE_AP) || (currentWiFiMode ==
  // WIFI_MODE_APSTA) || (currentWiFiMode == WIFI_MODE_NULL))
  ...
  if ((result != ESP_OK) && ((currentWiFiMode == WIFI_MODE_STA) ||
                             (currentWiFiMode == WIFI_MODE_APSTA)))
    Serial.printf("Failed to set STA MAC: %s | 0x%X\n",
                  macToString(this->sta_mac), result);
  // else if ((currentWiFiMode == WIFI_MODE_STA) || (currentWiFiMode ==
  // WIFI_MODE_APSTA))
...
void WiFiScan::RunBeaconScan(uint8_t scan_mode, uint16_t color) {
  ...
  // display_obj.setupScrollArea(display_obj.TOP_FIXED_AREA_2, BOT_FIXED_AREA);
  // else {
  // display_obj.setupScrollArea((STATUS_BAR_WIDTH * 2) + EXT_BUTTON_WIDTH,
  // BOT_FIXED_AREA);
...
/*void WiFiScan::RunStationScan(uint8_t scan_mode, uint16_t color) {
  ...
  if (err != ESP_OK) {
    //Serial.printf("Custom config failed (0x%04X), falling back to
default...\n", err); wifi_init_config_t default_cfg =
WIFI_INIT_CONFIG_DEFAULT(); err = esp_wifi_init(&default_cfg);
    //if (err != ESP_OK) {
...
void WiFiScan::RunRawScan(uint8_t scan_mode, uint16_t color) {
  ...
  display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
  // display_obj.setupScrollArea(display_obj.TOP_FIXED_AREA_2, BOT_FIXED_AREA);
  // display_obj.setupScrollArea((STATUS_BAR_WIDTH * 2) + CHAR_WIDTH - 1,
  // BOT_FIXED_AREA);
  ...
  if ((scan_mode == WIFI_SCAN_RAW_CAPTURE) ||
    ...
    display_obj.tftDrawExitScaleButtons(false);
    if (scan_mode == WIFI_SCAN_RAW_CAPTURE)
      display_obj.tftDrawChanHopButton(
          false, settings_obj.loadSetting<bool>("ChanHop"));
  }
#endif
#endif
...
// Serial.println("RunBluetoothScan");
  ...
  if ((scan_mode == BT_SCAN_FLOCK) || (scan_mode == BT_SCAN_FLOCK_WARDRIVE) ||
      ...
      (scan_mode == BT_SCAN_SIMPLE_TWO) ||
      (scan_mode == BT_SCAN_WAR_DRIVE_CONT) ||
      (scan_mode == BT_SCAN_ANALYZER) || (scan_mode == BT_SCAN_RAYBAN))
  ...
  if ((scan_mode == BT_SCAN_ALL) || (scan_mode == BT_SCAN_RAYBAN) ||
      ...
      (scan_mode == BT_SCAN_WAR_DRIVE_CONT) || (scan_mode == BT_SCAN_AIRTAG) ||
      (scan_mode == BT_SCAN_AIRTAG_MON) || (scan_mode == BT_SCAN_FLIPPER) ||
      (scan_mode == BT_SCAN_FLOCK) || (scan_mode == BT_SCAN_FLOCK_WARDRIVE) ||
...
//   return;
    ...
    if (scan_mode == BT_SCAN_WAR_DRIVE)
      ...
      display_obj.tft.drawCentreString("BT Wardrive Continuous", TFT_WIDTH / 2,
                                       16, 2);
  ...
  /*else if ((scan_mode == BT_SCAN_WAR_DRIVE) || (scan_mode ==
  BT_SCAN_WAR_DRIVE_CONT) || (scan_mode == BT_SCAN_FLOCK_WARDRIVE)) { #ifdef
  HAS_GPS if (gps_obj.getGpsModuleStatus()) { if (scan_mode ==
  BT_SCAN_WAR_DRIVE) { startLog("bt_wardrive");
        }
        else if (scan_mode == BT_SCAN_WAR_DRIVE_CONT) {
          startLog("bt_wardrive_cont");
        }
        ...
        String header_line = "WigleWifi-1.4,appRelease=" +
  (String)MARAUDER_VERSION + ",model=ESP32 Marauder,release=" +
  (String)MARAUDER_VERSION + ",device=ESP32 Marauder,display=SPI TFT,board=ESP32
    ...
    //  return;
      ...
      display_obj.tft.setTextColor(TFT_BLACK, color);
        ...
        if (scan_mode == BT_SCAN_WAR_DRIVE)
          ...
          display_obj.tft.drawCentreString("BT Wardrive Continuous",TFT_WIDTH /
  2,16,2); #ifdef HAS_ILI9341 display_obj.touchToExit(); #endif #endif
      display_obj.tft.setTextColor(TFT_CYAN, TFT_BLACK);
      display_obj.setupScrollArea(display_obj.TOP_FIXED_AREA_2, BOT_FIXED_AREA);
    #endif
    if (scan_mode != BT_SCAN_WAR_DRIVE_CONT)
      #ifndef HAS_NIMBLE_2
        pBLEScan->setAdvertisedDeviceCallbacks(new bluetoothScanAllCallback(),
  false); #else pBLEScan->setScanCallbacks(new bluetoothScanAllCallback(),
  false); #endif else #ifndef HAS_NIMBLE_2
        pBLEScan->setAdvertisedDeviceCallbacks(new bluetoothScanAllCallback(),
  true); #else pBLEScan->setScanCallbacks(new bluetoothScanAllCallback(), true);
      #endif
  ...
  if ((scan_mode == BT_SCAN_WAR_DRIVE_CONT) || (scan_mode == BT_SCAN_RAYBAN) ||
      (scan_mode == WIFI_SCAN_WAR_DRIVE) || (scan_mode == BT_SCAN_ANALYZER) ||
      (scan_mode == BT_SCAN_FLOCK) || (scan_mode == BT_SCAN_SIMPLE) ||
      (scan_mode == BT_SCAN_SIMPLE_TWO) ||
      (scan_mode == BT_SCAN_FLOCK_WARDRIVE))
...
uint8_t WiFiScan::getSecurityType(const uint8_t *beacon, uint16_t len) {
  ...
  while (i + 2 <= ies_len) {
    ...
    if (tag_id == 48) {
      ...
      if (tag_len >= 20 && tag_data[14] == 0x01 && tag_data[15] == 0x00 &&
          tag_data[16] == 0x00 && tag_data[17] == 0x0f &&
          tag_data[18] == 0xac) {
        isEnterprise = true;
      }
      ...
      if (tag_len >= 20 && tag_data[14] == 0x01 && tag_data[15] == 0x00 &&
          tag_data[16] == 0x00 && tag_data[17] == 0x0f &&
          tag_data[18] == 0xac && tag_data[19] == 0x08) {
        isWPA3 = true;
      }
    }
    ...
    // Check for WPA (in vendor specific tag)
      ...
      if (tag_len >= 20 && tag_data[14] == 0x01 && tag_data[15] == 0x00 &&
          tag_data[16] == 0x00 && tag_data[17] == 0x50 &&
          tag_data[18] == 0xf2) {
        isEnterprise = true;
      }
    }
    ...
    else if (tag_id == 221 && tag_len >= 4 && tag_data[0] == 0x00 &&
             tag_data[1] == 0x14 && tag_data[2] == 0x72 &&
             tag_data[3] == 0x01) {
      isWAPI = true;
    }
...
{ extern WiFiScan wifi_scan_obj; wifi_promiscuous_pkt_t *snifferPacket =
(wifi_promiscuous_pkt_t*)buf; int len = snifferPacket->rx_ctrl.sig_len;
...
CORRUPT_HEAP #ifdef HAS_SCREEN int buf = display_obj.display_buffer->size();
    #else
      int buf = 0;
    #endif
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0))
...
{ mac_match = false; break;
      ...
      if (!in_list) {
        ...
        // Get security info
        uint8_t security_type =
wifi_scan_obj.getSecurityType(snifferPacket->payload,
snifferPacket->rx_ctrl.sig_len);

        bool wps = wifi_scan_obj.beaconHasWPS(snifferPacket->payload,
snifferPacket->rx_ctrl.sig_len);
        ...
        AccessPoint ap = {essid,
                          ...
                          false,
                          {snifferPacket->payload[34],
snifferPacket->payload[35]}, snifferPacket->rx_ctrl.rssi, new
LinkedList<uint16_t>(), 0, security_type, wps};
      ...
      if (pinescan_match) {
        if (wifi_scan_obj.confirmed_pinescan->size() >= MAX_PINESCAN_ENTRIES) {
          if (!wifi_scan_obj.pinescan_list_full_reported) {
            ...
            Serial.println(
                F("Stopping PineScan detection until scan is restarted"));
            wifi_scan_obj.pinescan_list_full_reported = true;
          }
      ...
      if (ap_index == -1) {
        if (wifi_scan_obj.confirmed_multissid->size() >=
          ...
          if (!wifi_scan_obj.multissid_list_full_reported) {
            ...
            Serial.println(
                F("Stopping MultiSSID detection until scan is restarted"));
...
void WiFiScan::saeAttackLoop(uint32_t currentTime) {
  for (int i = 0; i < access_points->size(); i++) { // Find selected APs
    if (access_points->get(i).selected) {
      ...
      if (!this->sendSAECommitFrame(access_points->get(i).bssid,
                                    rand_source_mac)) {
...
/*bool WiFiScan::getSAEACT(const uint8_t *frame, size_t frame_len, uint16_t
&group_out, size_t &act_len_out, size_t &act_off_out) { act_len_out = 0;
  group_out = 0;
  act_off_out = 24 + 6 + 2;
  ...
  // If more than base, treat the extra as ACT length (optionally ignoring
trailing FCS). else if (rem > base) { size_t extra = rem - base;
...
// with spaces.
        ...
        { #ifdef HAS_GPS if (gps_obj.getGpsModuleStatus()) { bool do_save =
        false;

              // Check if we've already seen this AP
              char addr[] = "00:00:00:00:00:00";
              getMAC(addr, snifferPacket->payload, 10);
              if (wifi_scan_obj.seen_mac(reinterpret_cast<unsigned
        char*>(addr))) return;
        ...
        + gps_obj.getDatetime() + "," + (String)snifferPacket->rx_ctrl.channel +
        "," + (String)snifferPacket->rx_ctrl.rssi + "," + gps_obj.getLat() + ","
        + gps_obj.getLon() + "," + gps_obj.getAlt() + "," +
        gps_obj.getAccuracy() + ",WIFI"; Serial.println(wardrive_line);
                buffer_obj.append(wardrive_line);
              }
            }
          #endif
...
/*void WiFiScan::stationSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t
type) { extern WiFiScan wifi_scan_obj; wifi_promiscuous_pkt_t *snifferPacket =
(wifi_promiscuous_pkt_t*)buf; WifiMgmtHdr *frameControl =
(WifiMgmtHdr*)snifferPacket->payload; wifi_pkt_rx_ctrl_t ctrl =
(wifi_pkt_rx_ctrl_t)snifferPacket->rx_ctrl; int len =
snifferPacket->rx_ctrl.sig_len;
  ...
  for (int y = 0; y < 2; y++) {
    for (int i = 0; i < access_points->size(); i++) {
      ...
      for (int x = 0; x < 6; x++) {
        if (snifferPacket->payload[x + offsets[y]] !=
access_points->get(i).bssid[x]) { mac_match = false; break;
        }
      }
      if (mac_match) {
        matched_ap = true;
...
*)snifferPacket->payload; const WifiMgmtHdr *hdr = &ipkt->hdr;
    ...
    if (snifferPacket->payload[0] == 0x40) // Probe request
      ...
      wifi_scan_obj.deauth_frames++;
    else if (((snifferPacket->payload[30] == 0x88 && snifferPacket->payload[31]
== 0x8e) || ( snifferPacket->payload[32] == 0x88 && snifferPacket->payload[33]
== 0x8e))) // eapol wifi_scan_obj.eapol_frames++;
  ...
  if (wifi_scan_obj.currentScanMode == WIFI_SCAN_SIG_STREN) {
    ...
    if ((targ_ap.rssi + 1 < snifferPacket->rx_ctrl.rssi) ||
(snifferPacket->rx_ctrl.rssi + 1 < targ_ap.rssi)) { targ_ap.rssi =
snifferPacket->rx_ctrl.rssi; access_points->set(targ_index, targ_ap);

      Serial.println((String)access_points->get(targ_index).essid + " RSSI: " +
(String)access_points->get(targ_index).rssi);
    }
    else
      return;
  }
...
/*void WiFiScan::deauthSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t
type)
{
  wifi_promiscuous_pkt_t *snifferPacket = (wifi_promiscuous_pkt_t*)buf;
  WifiMgmtHdr *frameControl = (WifiMgmtHdr*)snifferPacket->payload;
...
/*void WiFiScan::probeSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t
type) {
...
CORRUPT_HEAP #ifdef HAS_SCREEN int buf = display_obj.display_buffer->size();
    ...
    if ((snifferPacket->payload[0] == 0x40) && (buf == 0))
    {
      if (wifi_scan_obj.currentScanMode == WIFI_SCAN_PROBE) {
        ...
        // Print spaces because of the rotating lines of the hardware scroll.
        // The same characters print from previous lines so I just overwrite
them
        // with spaces.
        #ifdef HAS_SCREEN
          for (int i = 0; i < 19 - snifferPacket->payload[25]; i++)
...
",WIFI"; Serial.println(wardrive_line); buffer_obj.append(wardrive_line);
            }
          }
        #endif
      }
...
wifi_promiscuous_pkt_type_t type) { wifi_promiscuous_pkt_t *snifferPacket =
(wifi_promiscuous_pkt_t*)buf; WifiMgmtHdr *frameControl =
(WifiMgmtHdr*)snifferPacket->payload; wifi_pkt_rx_ctrl_t ctrl =
(wifi_pkt_rx_ctrl_t)snifferPacket->rx_ctrl; int len =
snifferPacket->rx_ctrl.sig_len;
...
/*void WiFiScan::sendDeauthFrame(uint8_t bssid[6], int channel, String
dst_mac_str) {
  // Itterate through all access points in list
  // Check if active
  WiFiScan::set_channel = channel;
...
&deauth_frame_default[6], &deauth_frame_default[7], &deauth_frame_default[8],
&deauth_frame_default[9]);
  ...
  // Send packet
  esp_wifi_80211_tx(WIFI_IF_AP, deauth_frame_default,
sizeof(deauth_frame_default), false); esp_wifi_80211_tx(WIFI_IF_AP,
deauth_frame_default, sizeof(deauth_frame_default), false);
  esp_wifi_80211_tx(WIFI_IF_AP, deauth_frame_default,
sizeof(deauth_frame_default), false);
...
void WiFiScan::sendEapolBagMsg1(uint8_t bssid[6], int channel,
                                String dst_mac_str, uint8_t sec) {
  WiFiScan::set_channel = channel;
  this->changeChannel(channel);
  delay(1);
  ...
  sscanf(dst_mac_str.c_str(), "%2hhx:%2hhx:%2hhx:%2hhx:%2hhx:%2hhx",
         ...
         &eapol_packet_bad_msg1[6], &eapol_packet_bad_msg1[7],
         &eapol_packet_bad_msg1[8], &eapol_packet_bad_msg1[9]);
...
/*void WiFiScan::sendAssociationSleep(const char* ESSID, uint8_t bssid[6], int
channel, String dst_mac_str) { WiFiScan::set_channel = channel;
  this->changeChannel(channel);
  delay(1);
  ...
  // Build packet
  sscanf(dst_mac_str.c_str(), "%2hhx:%2hhx:%2hhx:%2hhx:%2hhx:%2hhx",
        &eapol_packet_bad_msg1[10], &eapol_packet_bad_msg1[11],
&eapol_packet_bad_msg1[12], &eapol_packet_bad_msg1[13],
&eapol_packet_bad_msg1[14], &eapol_packet_bad_msg1[15]);
...
wifi_promiscuous_pkt_type_t type) { bool is_beacon = false; bool is_eapol =
false; extern WiFiScan wifi_scan_obj;
    ...
    if (wifi_scan_obj.currentScanMode == WIFI_SCAN_ACTIVE_LIST_EAPOL) {
      ...
      for (int i = 0; i < access_points->size(); i++) {
        if (access_points->get(i).selected) {
          ...
          if (found) {
            Serial.println("Received beacon from " + access_points->get(i).essid
+ ". Deauthenticating..."); break;
          }
        }
      }
      if (!found)
    ...
    // Send packet
    esp_wifi_80211_tx(WIFI_IF_AP, wifi_scan_obj.deauth_frame_default,
sizeof(wifi_scan_obj.deauth_frame_default), false); delay(1);
  }



  if (( (snifferPacket->payload[30] == 0x88 && snifferPacket->payload[31] ==
0x8e)|| ( snifferPacket->payload[32] == 0x88 && snifferPacket->payload[33] ==
0x8e) )){ is_eapol = true; num_eapol++; Serial.println(F("Received EAPOL:"));
...
COMPENSATE FOR BUTTONS AND COLOR KEY SO 'ERASER' DOESN'T ERASE BUTTONS AND COLOR
KEY!!! if ((x_pos <= 90) || ((x_pos >= 117) && (x_pos <= 320))) //above x axis
        display_obj.tft.fillRect(x_pos+1, 28, 10, 93, TFT_BLACK); //compensate
for buttons! else display_obj.tft.fillRect(x_pos+1, 0, 10, 121, TFT_BLACK);
//don't compensate for buttons!
  ...
  display_obj.tft.fillRect(127, 0, 193, 28, TFT_BLACK); //erase XY buttons and
any lines behind them display_obj.tft.fillRect(12, 0, 90, 32, TFT_BLACK); // key
  display_obj.tftDrawChannelScaleButtons(set_channel);
  display_obj.tftDrawExitScaleButtons();
  display_obj.tftDrawEapolColorKey(this->filterActive());
...
#ifdef HAS_SCREEN
  // #ifdef HAS_FULL_SCREEN
  display_obj.tft.fillRect(0, TFT_HEIGHT - GRAPH_VERT_LIM - (CHAR_WIDTH * 2),
                           TFT_WIDTH, (CHAR_WIDTH * 2) - 1, TFT_BLACK);
// #else
// #endif
// Serial.println("Drawing channel line...");
  ...
  for (int i = 1; i < CHAN_PER_PAGE + 1; i++) {
    ...
    display_obj.tft.setTextSize(1);
#endif
    display_obj.tft.setCursor(x_coord,
                              TFT_HEIGHT - GRAPH_VERT_LIM - (CHAR_WIDTH * 2));
    display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
    display_obj.tft.print(
...
void WiFiScan::main(uint32_t currentTime) {
  ...
  /*else if ((currentScanMode == WIFI_ATTACK_MIMIC)) {
    ...
    for (int i = 0; i < access_points->size(); i++) {
      if (access_points->get(i).selected) {
        #ifndef HAS_DUAL_BAND
          this->broadcastCustomBeacon(currentTime,
  ssid{access_points->get(i).essid, random(1, 12), {random(256), random(256),
                                                                                                    random(256),
                                                                                                    random(256),
                                                                                                    random(256),
                                                                                                    random(256)}});
        #else
          this->broadcastCustomBeacon(currentTime,
  ssid{access_points->get(i).essid, dual_band_channels[random(0,
  DUAL_BAND_CHANNELS)], {random(256), random(256), random(256), random(256),
                                                                                                    random(256),
                                                                                                    random(256)}});
        #endif
      }
