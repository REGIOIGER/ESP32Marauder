#line 1 "/home/calvin/EC/ESP32Marauder/esp32_marauder/GpsInterface.cpp"
...
String GpsInterface::generateGXgga() {
  ...
  char timeStr[8];
  snprintf(timeStr, 8, "%02d%02d%02d,", (int)(nmea.getHour()),
           (int)(nmea.getMinute()), (int)(nmea.getSecond()));

  long lat = nmea.getLatitude();
  char latDir = lat < 0 ? 'S' : 'N';
  lat = abs(lat);
  char latStr[12];
  snprintf(latStr, 12, "%02ld%08.5f,", lat / 1000000,
  ...
  lon = abs(lon);
  char lonStr[13];
  snprintf(lonStr, 13, "%03ld%08.5f,", lon / 1000000,
  ...
  String message = msg_type + timeStr + latStr + latDir + ',' + lonStr +
...
String GpsInterface::generateGXrmc() {
  ...
  String message = msg_type + timeStr + status + ',' + latStr + latDir + ',' +
                   lonStr + lonDir + ',' + speedStr + courseStr + dateStr +
                   ',' + ',' + mode;
  return message;
}
...
String GpsInterface::generateType() {
  ...
  if (this->type_flag == GPSTYPE_NATIVE) { // type_flag=0
    ...
    msg_type += 'B';
    msg_type += 'D';
  } else { // type_flag=1=all ... also default if unset/wrong (obj default is
           // type_flag=0=native)
    if (this->type_flag >=
        8) // catch uncaught first char, assume G if not already output
      msg_type += 'G';
    msg_type += 'N';
  }
...
float GpsInterface::getAccuracy() { return this->accuracy; }

String GpsInterface::getLat() { return this->lat; }

String GpsInterface::getLon() { return this->lon; }

int32_t GpsInterface::getLatInt() { return this->lat_int; }

int32_t GpsInterface::getLonInt() { return this->lon_int; }

float GpsInterface::getAlt() { return this->altf; }
